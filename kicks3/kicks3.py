...
def put_bucket_policy(bucket):
    try:
        ...
        bucket_policy = {'Version': '2012-10-17', 'Statement': [
            {'Sid': 'AddPerm', 'Effect': 'Allow', 'Principal': '*', 'Action': 's3:*',
...
def scan_s3(f, silent=False):
    ...
    for line in f:
        ...
        if not silent:
            ...
            print('Bucketname - ' + line, 'unauth_list - ' + str(listing[0]), 'auth_list - ' + str(listing[1]),
                  'auth_write - ' + str(upload), 'get-bucket-acl' + str(get_acl), +str(put_acl))
            s3out.write('Bucketname - ' + line + ',' + 'unauth_list - ' + str(listing[0]) + ',' + 'auth_list - ' + str(
                listing[1]) + ',' + 'auth_write - ' + str(upload) + ',' + 'get-bucket-acl - ' + str(
                get_acl) + ',put_bucket_policy - ' + str(put_acl) + '\n')
            s3out.close()
